...
class StyleGAN_Wrapper(torch.nn.Module):
    ...
    def forward(self, ws=None, f_latents=None, f_layer=0, mode="wp"):
        ...
        if mode == "wp":
            ...
            return self.forward_from_to(ws, f_latents, f_layer)
    ...
    def forward_from_to(self, ws, f_latents, f_layer, **layer_kwargs):
        misc.assert_shape(ws, [None, self.syn.num_ws, self.syn.w_dim])
        ws = ws.to(torch.float32).unbind(dim=1)

        x = f_latents
        # for name, w in zip(self.syn.layer_names[f_layer: f_layer + 2], ws[f_layer + 1: f_layer + 3]):

        #     x = getattr(self.syn, name)(x, w, **layer_kwargs)
        x = getattr(self.syn, self.syn.layer_names[f_layer])(x, ws[f_layer + 1], **layer_kwargs)
        # Ensure correct shape and dtype.
        # if self.syn.output_scale != 1:
            # x = x * self.syn.output_scale
